...
app.get(
  "/bookings/:startDay/:startMonth/:endDay/:endMonth",
  async (req, res) => {
    ...
    if (startMonth === endMonth) {
      const x = await hotelBooking.find({
        arrival_date_month: startMonth,
        arrival_date_day_of_month: { $gte: startDay, $lte: endDay },
      });
      res.json(x);
    } else {
      const x = await hotelBooking.find({
        $or: [
          {
            ...
            arrival_date_day_of_month: { $gte: startDay, $lte: 31 },
          },
          {
            arrival_date_month: "August",
            arrival_date_day_of_month: { $gte: 1, $lte: endDay },
          },
        ],
      });
      res.json(x);
